---
title: 01.05 File Management
date: 2025-08-28T09:00:00Z
lastmod: 2025-08-29T07:51:24
---

When consuming content it is not important to think about different digital file types. The bitrate or video codec used for an online video stream is only relevant to the perceived quality of the product. But when creating content file management and digital asset management are an essential part of the workflow.

No single way of managing files is "correct" but there are some generally accepted best practices.
