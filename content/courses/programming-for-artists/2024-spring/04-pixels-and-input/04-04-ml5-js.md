...
When handpose detects a hand it returns results which is a array of different objects that contain arrays of the coordinates of points on the hand in x ,y, and z space. To see this array, use `if(predictions.length > 0) {console.log(predictions);}` in your code. Wait for a hand to be detected and then look at the `predictions` array in the console log.
